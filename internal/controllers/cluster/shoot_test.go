...
var _ = Describe("Shoot Logic", func() {
	...
	Context("UpdateShootFields", func() {
		...
		It("should update the shoot fields with the expected values", func() {
			...
			hasMetadataAnnotation := false
			for k, v := range c.Annotations {
				if strings.HasPrefix(k, openmcpconst.MetadataAnnotationLabelPrefix) {
					Expect(shoot.Annotations).To(HaveKeyWithValue(k, v))
					hasMetadataAnnotation = true
				}
			}
			Expect(hasMetadataAnnotation).To(BeTrue(), "there should be at least one metadata annotation on the cluster for this check to be effective")
			...
			Expect(shoot.Spec.Extensions).To(ContainElements(
				MatchFields(0, Fields{
					...
					"ProviderConfig": PointTo(MatchFields(IgnoreExtras, Fields{
						"Raw": Equal([]byte(`{"foo":"foobar"}`)),
					})),
				}),
				MatchFields(0, Fields{
					"Type":     Equal("new-extension"),
					"Disabled": PointTo(BeTrue()),
					"ProviderConfig": PointTo(MatchFields(IgnoreExtras, Fields{
						"Raw": Equal([]byte(`{"bar":"baz"}`)),
					})),
				}),
			))
